/* Firmware for steering wheel task
 * Tony Hyun Kim
 * kimth@stanford.edu
 */
...
void setup() {
  ...
  // Set up Timer1 to output 120 Hz clock for Imaging Source camera
  // See: https://forum.arduino.cc/index.php?topic=72092.0
  pinMode(BEHAVIOR_CLOCK, OUTPUT);
  TCCR1B &= ~7; // Clear prescaler bits
  TCCR1B |= 4; // Set prescaler=4
  analogWrite(BEHAVIOR_CLOCK, 0);
}
...
void loop() {
  ...
  if (Serial.available() >0) {
    ...
    switch (s) {
      ...
      if ((s>40 && s<90) || (s>90 && s!=340 && s!=370 && s!=380 && s!=390 && s!=400 && s!=410)) {
        s=-1;
      }
	...
	Serial.println(agv);       /* send value via serial  */
      ...
      /* THK: Low-level functions for steering wheel task */

      case 370: /* set_screen_ttl */
        if (val)
          digitalWrite(SCREEN_TTL, 1);
        else
          digitalWrite(SCREEN_TTL, 0);
      ...
      case 400: /* set_behavior_clock */
        if (val)
          analogWrite(BEHAVIOR_CLOCK, 128); // 50% duty cycle
        else
          analogWrite(BEHAVIOR_CLOCK, 0);

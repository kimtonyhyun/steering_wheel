classdef arduino < handle
    ...
    methods
        ...
        function delete(a)
            ...
            if isa(a.aser,'serial') && isvalid(a.aser) && strcmpi(get(a.aser,'Status'),'open'),
                % THK: Unnecessary, and causes input pins to float high
...
%                 end
        ...
        % THK
        %------------------------------------------------------------
        function dispense(a, duration_per_pulse_ms, num_pulses)
            fwrite(a.aser,[89, duration_per_pulse_ms, num_pulses], 'uchar');
        end % send_pulse
        
        function set_behavior_clock(a, val)
            fwrite(a.aser,[88, val], 'uchar');
        end % set_behavior_clock
        
        function start_behavior_clock(a)
            a.set_behavior_clock(1);
        end
        
        function stop_behavior_clock(a)
            a.set_behavior_clock(0);
        end
        
        function set_screen_ttl(a, val)
            fwrite(a.aser,[85, val], 'uchar');
        end
        
        function set_stim_ttl(a, val) % added on 07/24/2023, use case 60 on the arduino code 
            fwrite(a.aser,[60, val], 'uchar');
        end
        function set_stim_ttl2(a, val) % added on 07/24/2023, use case 60 on the arduino code 
            fwrite(a.aser,[61, val], 'uchar');
        end
        function set_stim_ttl3(a, val) % added on 07/24/2023, use case 60 on the arduino code 
            fwrite(a.aser,[62, val], 'uchar');
        end
        function set_stim_ttl4(a, val) % added on 07/24/2023, use case 60 on the arduino code 
            fwrite(a.aser,[63, val], 'uchar');
        end
